...
本模块实现了论坛系统的路由和视图函数，包括：
1. 论坛首页：显示所有主题列表，支持分类筛选和分页
2. 主题详情：显示主题内容和回复，支持嵌套回复
3. 创建主题：创建新的讨论主题
4. 主题管理：置顶、关闭、删除主题
5. 回复管理：发表回复、回复特定评论、删除回复

论坛系统支持以下特性：
- 嵌套回复：支持对特定评论进行回复，形成对话
- 实时更新：通过WebSocket实时推送新回复
- 通知集成：回复时自动发送通知给相关用户
- 权限控制：基于用户角色和身份的权限控制
- 性能优化：使用JOIN查询减少数据库查询次数
"""
...
def topic(id):
    ...
    模板上下文:
        topic: 主题信息，包含标题、分类、创建者等
        posts: 回复列表，包含作者信息和嵌套回复
        pagination: 分页对象，用于生成分页控件
        form: 回复表单
...
def create_topic():
    ...
    模板上下文:
        form: 创建主题表单，包含标题、分类和内容字段

    数据库操作:
        - 创建ForumTopic记录，包含标题、分类和创建者
        - 创建ForumPost记录，作为主题的第一个帖子
        - 使用事务确保两个操作同时成功或失败
    """
    form = TopicForm()
